...
function json_encodes($str){
        ...
        return preg_replace("#\\\u([0-9a-f]{4})#i", function (){return "iconv('UCS-2', 'UTF-8', pack('H4', '\\1'))";}, $code);

    }
function safe_b64encode($data) {
        $data = str_replace('+','----------------------------------------------',$data);
        $data = str_replace('/','_________________________________________',$data);
        $data = str_replace('=','````````````````',$data);
        return $data;
    }
//解析特殊字符
function safe_b64decode($string) {
    ...
    $data = str_replace('_________________________________________','/',$string);
    $data = str_replace('````````````````','=',$string);
    $mod4 = strlen($data)%4;
    if ($mod4) {
        $data .= substr('====', $mod4);
...
//    while (1) {
//        $count = db('order')->where("$field", $billno)->count('id');
//        if ($count <= 0) {
//            break;
//        }
