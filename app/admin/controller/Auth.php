...
//            $map = '';
//            if ($key) {
//                $map['loginname'] = array('like', "%" . $key . "%");
//            }
            if (session('aid') != 1) {
                $map['id'] = session('aid');
            }

            $list = Db::table(config('database.prefix') . 'admin')
                ->alias('a')
                ->field('a.*');
            if (isset($where['group_id']) && $where['group_id'] > 0){
                $list->where('find_in_set(\''.$where['group_id'].'\',group_id)');
            }
            if ($key){
                $list->where('loginname|mobile|username', 'like', "%" . $key . "%");
            }
            if (session('aid') != 1) {
                $list->where(['a.admin_id' => session('aid')]);
            }
            ...
            foreach ($list['data'] as $k => $v) {
                ...
                $list['data'][$k]['birthday'] = date('Y-m-d', $v['birthday']);
                $group_title = db('auth_group')->where(['group_id' => ['in',explode(',',$v['group_id'])]])->column('title');
                $list['data'][$k]['title'] = implode(',',$group_title);
                if($list['data'][$k]['sex']){
                    $list['data'][$k]['sex'] = '女';
                }else{
                    $list['data'][$k]['sex'] = '男';
    ...
    public function adminAdd()
    {
        if (request()->isPost()) {
            ...
            if(empty($data['group_id'])){
                return $result = ['code' => 0, 'msg' => '权限不能为空！'];
            }else{
                $data['group_id'] = implode(',',$data['group_id']);
            }

            //$data['birthday']=strtotime($data['birthday']);
            $check_user = Admin::get(['loginname' => $data['loginname']]);
            if ($check_user) {
    ...
    public function adminEdit()
    {
        if (request()->isPost()) {
            ...
            if (input('post.username')) {
                $map['loginname'] = input('post.loginname');
            ...
            $avatar = $data['img'] == '' ? '/static/admin/images/0.jpg' : $data['img'];
            ...
            //$info['birthday']=date('Y-m-d H:i:s', $info['birthday']);
            $selected = db('auth_group')->where(['group_id' => ['in',explode(',',$info['group_id'])]])->column('group_id');
            $this->assign('group', $auth_group);
            $this->assign('selected', $selected);
            $this->assign('info', $info->toJson());
            $this->assign('info_1', $info);
    ...
    public function groupEdit()
    {
        if (request()->isPost()) {
            ...
            $result = ['code' => 1, 'msg' => '角色修改成功!', 'url' => url('adminGroup')];
            return $result;
        } else {
            $id = input('id');
    ...
    {
        $nav = new Leftnav();
        $admin_rule = db('auth_rule')->where(['menustatus' => 1])->field('id,pid,title')->order('sort asc')->select();
        $rules = db('auth_group')->where('group_id', input('id'))->value('rules');
        $arr = $nav->auth($admin_rule, $pid = 0, $rules);
        $arr[] = array(
            "id" => 0,
    ...
    public function adminRule()
    {
        if (request()->isPost()) {
            ...
            if (!$arr) {
                $authRule = AuthRule::where(['menustatus' => 1])->select(function ($query) {
